from losses import range_loss, d_clip_loss, d_clip_dir_loss, mse_loss, get_features, zecon_loss_direct
...
class LossCal:
    def __init__(self) -> None: 
        ...
        self.image_augmentations = ImageAugmentations(224, 0.5, 0.5, 1.0, patch=False)
        self.patch_augmentations = ImageAugmentations(224, 0.5, 0.5, 1.0, patch=True)
        self.clip_model = (
        ...
        self.vgg = models.vgg19(pretrained=True).features.to(self.device).eval().requires_grad_(False)
    ...
    def clip_global_loss_text(self, image, text): # calculate the loss between image and text
    ...
    def clip_global_loss_image(self, image, style_image) -> float:
        ...
        augmented_style = self.image_augmentations(style_image, 1).add(1).div(2)
